...
const Drills = () => {
  ...
  const handleDrillComplete = async (completionData) => {
    try {
      await axiosInstance.post(`/drills/complete`, {
  ...
  useEffect(() => {
    const fetchDrills = async () => {
      try {
        const { data } = await axiosInstance.get(`/drills/user/${user?._id}`);
        const normalized = Array.isArray(data)
          ? data.map((drill) => ({
              ...
              location: drill.location || drill.region || "All Regions",
              scheduledTime: drill.scheduledTime || drill.date,
              completed: drill.status === "completed",
            }))
          : [];
        setDrills(normalized);
      } catch (err) {
        console.error("Failed to fetch drills from API", err);
        setDrills([]);
      } finally {
        setLoading(false);
      }
