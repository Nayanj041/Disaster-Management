    ...
    const fetchProgress = async () => {
      try {
        const [progressRes, leaderboardRes] = await Promise.all([
          axiosInstance.get(`/gamification/${user._id}/progress`),
        ...
        const mappedLeaderboard = (Array.isArray(leaderboardRes.data)
          ? leaderboardRes.data
          : []
        )
          .map((row) => ({
        ...
        setLeaderboardUsers(mappedLeaderboard);
      } catch (err) {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `#${
        Math.max(
          1,
          leaderboardUsers.findIndex((u) => u.id === user?._id) + 1
        )
      }`,
      icon: Target,
      color: "green",
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {activeTab === "leaderboard" && (
          <div className="space-y-6">
            <Leaderboard users={leaderboardUsers} currentUserId={user?._id} />
          </div>
        )}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {userProgress.earnedBadges?.length
                  ? `${userProgress.earnedBadges.length} Achievements Earned`
                  : "No Achievements Yet"}
              </h3>
              <p className="text-gray-600">
                Complete modules and drills to unlock new achievements.
              </p>
            </div>
          </div>
        )}
